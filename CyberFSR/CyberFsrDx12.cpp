		...
		*/
		if (!config->DisableReactiveMask.has_value() || (config->DisableReactiveMask.has_value() && (config->DisableReactiveMask.chars() == false)))
		{
			if (inParams->InputBiasCurrentColorMask != nullptr) {
				...
				// Set the required values for the automatic generation feature
				dispatchParameters.autoTcThreshold = 0.05f;  // Recommended default value 0.05f
				dispatchParameters.autoTcScale = 1.0f;      // Recommended default value 1.00
				dispatchParameters.autoReactiveScale = 5.00f;  // Recommended default value 5.00
				dispatchParameters.autoReactiveMax = 0.95f;  // Recommended default value 0.90
			}
			else {
				dispatchParameters.reactive = ffxGetResourceDX12(fsrContext, nullptr, L"FSR2_EmptyInputReactiveMap");
			}
