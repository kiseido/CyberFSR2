...
{
	CyberFsrContext::instance()->paramFactory.DestroyParameter((Hyper_NGX_Parameter::Parameter*)InParameters);
	return NVSDK_NGX_Result_Success;
}
...
{
	const auto inParams = Hyper_NGX_Parameter::Parameter::convertPointerIfValid(InParameters);
	...
	* OutHandle = &deviceContext->Handle;
	...
	initParams.maxRenderSize.height = Hyper_NGX_Parameter::ConvertVariant<uint32_t>(inParams->parameterDB.Get(NGX_Strings::NVSDK_NGX_Parameter_Width_enum)).chars();
	initParams.displaySize.width = Hyper_NGX_Parameter::ConvertVariant<uint32_t>(inParams->parameterDB.Get(NGX_Strings::NVSDK_NGX_Parameter_OutWidth_enum)).chars();
	initParams.displaySize.height = Hyper_NGX_Parameter::ConvertVariant<uint32_t>(inParams->parameterDB.Get(NGX_Strings::NVSDK_NGX_Parameter_OutHeight_enum)).chars();

	initParams.flags = 0;
	if (config->DepthInverted.value_or(Hyper_NGX_Parameter::ConvertVariant<uint32_t>(inParams->parameterDB.Get(NGX_Strings::NVSDK_NGX_Parameter_Width_enum)).chars()))
	{
...
NVSDK_NGX_Result NVSDK_NGX_D3D11_EvaluateFeature(ID3D11DeviceContext* InDevCtx, const NVSDK_NGX_Handle* InFeatureHandle, const NVSDK_NGX_Parameter* InParameters, PFN_NVSDK_NGX_ProgressCallback InCallback)
{
	...
	const auto inParams = static_cast<const Hyper_NGX_Parameter::Parameter*>(InParameters);
